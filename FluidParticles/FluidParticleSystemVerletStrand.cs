...
public class FluidParticleSystemVerletStrand : FluidParticleSystem {
    ...
    public FluidParticleSystemVerletStrand(Transform a, Vector3 localPointA, Transform b, Vector3 localPointB,
        Material material, FluidParticleSystemSettings fluidParticleSystemSettings, LayerMask collisionLayerMask,
        int particleCountMax = 25) :
        ...
        for (int i = 0; i < _particles.Length; i++) {
            ...
            if (progress < 0.5f) {
                _particles[i].size =
                    baseSize * fluidParticleSystemSettings
                        .particleBaseSize; //((1f-Easing.OutPower(progress * 2f, 5))*cubicSize + baseSize) * fluidParticleSystemSettings.particleBaseSize;
                _particles[i].color = fluidParticleSystemSettings.color;
                float easing = 1f - Easing.OutPower(progress * 2f, 4);
                _particles[i].heightStrength = (baseHeightStrength + (easing * cubicHeightStrength)) *
                                               fluidParticleSystemSettings.heightStrengthBase;
            } else {
                _particles[i].size =
                    baseSize * fluidParticleSystemSettings
                        .particleBaseSize; // ((Easing.InPower((progress-0.5f) * 2f, 5))*cubicSize + baseSize) * fluidParticleSystemSettings.particleBaseSize;
                _particles[i].color = fluidParticleSystemSettings.color;
        ...
        for (var i = 1; i < _particles.Length - 1; i++) {
            ...
            if (broken) {
                ...
                _particles[i].position +=
                    vel * realFriction + Physics.gravity * (dt * dt * gravityMult);
            }
        }
    }
}
